 ...
 *
 * Notes:
 * - This module keeps the player's **server/client rotation frozen** at the moment of activation.
 * - Camera yaw/pitch are handled by changeLookDirection(...) which Meteor's input system calls.
 * - Mining is performed by the real player via packets (START/STOP_DESTROY_BLOCK), using the player's raycast.
 */
public class FreecamV2 extends Module {
    ...
    private void onTick(TickEvent.Post event) {
        ...
        if (playerMine.get()) {
            ...
            if (attacking && !wasMining) {
                try {
                    var hr = mc.player.raycast(6.0, 0f, false); // raycast from player (rotation frozen)
                    if (hr instanceof BlockHitResult bhr) {
    ...
    /**
     * Called by Meteor's input handling when the mouse moves.
     * This lets the freecam look independently of the player (player rotation stays frozen).
     */
    public void changeLookDirection(double deltaX, double deltaY) {
        lastYaw = yaw;
        lastPitch = pitch;

        // Mouse deltas - invert/scale as needed by your Meteor version.
        // These deltas come from Meteor's input handler; tweak signs if it feels backwards.
        yaw += (float) deltaX;
        pitch += (float) deltaY;
